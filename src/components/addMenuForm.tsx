...
const AddMenuForm = () => {
	...
	const handleIngredientSelect = (ingredient: Ingredient) => {
		if (
			!selectedIngredients.some((item) => item.bahan_id === ingredient.bahan_id)
		) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			const response = await axios.post(
				"http://192.168.77.90:5000/api/menu",
				payload,
				{
					headers: {
						Authorization: `Bearer ${token}`
	...
	return (
		<div className="space-y-6">
			<div>
				<h2 className="text-2xl font-bold">Insert New Menu</h2>
				<p className="text-muted-foreground">
					Fill these to make your desired menu
				</p>
			</div>
					...
					<select
						...
						{ingredients.length ? (
							ingredients.map((ingredient) => (
								<option
									key={`ingredient-${ingredient.bahan_id}`}
									value={ingredient.bahan_id}>
									{ingredient.nama}
								</option>
							))
						) : (
